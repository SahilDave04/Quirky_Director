...
class DisplayTree:
    ...
    def __init__(
        ...
        :param exportTxt: Boolean Flag to Export Directory Tree to TXT File
        ...
        self.exportTxt: bool = exportTxt

    # Destructor
    def __del__(self):

        del self.dirPath, self.stringRep, self.header, self.maxDepth, self.showHidden, self.ignoreList, \
            self.onlyFiles, self.onlyDirs, self.sortBy, self.raiseException, self.printErrorTraceback, self.exportTxt
    ...
    def displayTree(self) -> Union[str, None]:
        ...
        try:
            ...
            if self.exportTxt:
                if "Directory_Tree.txt" in listdir(getcwd()):
                    open("Directory_Tree.txt", "w").close()
                
                f = open("Directory_Tree.txt", "a")
                for path in paths:
                    f.write(path.displayPath()+"\n")
                f.close()
